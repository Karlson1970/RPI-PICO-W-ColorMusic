...
#define ENCODER_PIN_A   15    // пин подключения энкодера канал A
#define ENCODER_PIN_B   14    // пин подключения энкодера канал B
#define ENCODER_BUTTON  13    // пин подкдючения кнопки энкодера
...
// Функция рисует столбик на экране с заданой высотой x(номер столбца), y (0-100)
...
void showWaveform() {          // Показываем форму сигнала
  for (int i = 0; i < 52; i++) {
    u8g2.drawLine(PX1 - i, PY1 - (waveCh1[i * 2]) / 256, PX1 - i - 1, 16 - (waveCh1[i * 2 + 1] / 256)); // Первый канал
    u8g2.drawLine(PX2 + i, PY1 - (waveCh2[i * 2]) / 256, PX2 + i + 1, 16 - (waveCh2[i * 2 + 1] / 256)); // Второй канал
  }
}
...
// аппаратные прерывания от энкодера канал A и канал B
  ...
  attachInterrupt(digitalPinToInterrupt(ENCODER_PIN_A), encoderA, RISING);
  attachInterrupt(digitalPinToInterrupt(ENCODER_PIN_B), encoderB, RISING);
